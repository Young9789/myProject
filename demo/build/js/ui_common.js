(function(window, document, jQuery) {
	...
	shopUi.mainGallery = {
		init: function(){
			...
			mainSwiper = new Swiper($target, {
				...
				onInit: function(swiper){
					...
					// shopUi.mainGallery.resize($target);
				}
			});
		...
		resize: function($banner){
			var	$hdrann = $('#hdrann');
			var winHeight = $window.height(),
				hdrannVisible = $hdrann.is(':visible'),
				headerHeight = $('#gHeader').height(),
				hdrannHeight = $hdrann.height(),
				minHeight = 750,
				bannerHeight,
				wrapHeight;
			...
			bannerHeight = winHeight;
			wrapHeight = winHeight - headerHeight;
			
			if( hdrannVisible ){
				bannerHeight -= hdrannHeight;
				wrapHeight -= hdrannHeight;
			}
			$banner.css({
				'margin-top': -1 * headerHeight,
				'height': bannerHeight
			});
			$('#mainBannerWrap').height( wrapHeight );
		}*/
	...
	shopUi.dimmed = {
		...
		show: function(duration){
			...
			$dimmed
				.css('z-index', shopUi.layerDepth-1)
				.stop().fadeIn(runTime);
		},
		...
		update: function(){
			var $dimmed = $('#wrapDimmed');
			$dimmed.css('z-index', shopUi.layerDepth-1);
		}
	};
	...
	shopUi.sideCart = {
		...
		show: function(){
			...
			this.obj.css('z-index',shopUi.layerDepth += 2);
			...
			if( useTransform ){
				this.obj.stop().animate({ borderSpacing: -340 }, {
				...
				this.obj.stop().animate({ 'right': '340px' }, this.runTime);
			}
		...
		hide: function(){
			...
			var completeFunc = function(){
				...
				_this.obj.css('z-index',shopUi.layerDepth -= 2);
			};
			...
			if( useTransform ){
				this.obj.stop().animate({ borderSpacing: 340 }, {
					step: function(now,fx) {
						$(this).css(useTransform, 'translateX('+ (now - 340) +'px)');
					},
	...
	shopUi.showProductLayer = function(){
		$body.on('click', '.js_quick_view', function(){
			...
			shopUi.layerCtrl.show( $('#productLayer') );
			// shopUtil.windowScrollbar.disable();
			// shopUi.dimmed.show();
		});
		$body.on('click', '.js_quick_close', function(){
			$('#productLayer').fadeOut(500);
			shopUi.layerCtrl.hide( $('#productLayer') );
			// shopUtil.windowScrollbar.enable();
			// shopUi.dimmed.hide();
		});
	};
	...
	shopUi.layerCtrl = {
		...
		show: function( obj ){
			if( obj.length > 0 ){
				shopUi.openedLayer.push(obj);

				obj.css('z-index', shopUi.layerDepth += 2);
				obj.fadeIn();
				shopUi.dimmed.show();
				shopUtil.windowScrollbar.disable();
			}
		...
		hide: function(){
			var openedLength = shopUi.openedLayer.length-1,
				lastLayer = shopUi.openedLayer[openedLength];
			if( lastLayer ){
				lastLayer.fadeOut();
				shopUi.openedLayer.pop();
				shopUi.layerDepth -= 2;
			}
			if( openedLength > 0 ){
				shopUi.dimmed.update();
			}else{
				shopUi.dimmed.hide();
				shopUtil.windowScrollbar.enable();
			}
	...
	shopUi.mainProduct = {
		init: function(){
			...
			var $target = $('#mainProduct'),
				nextBtnSelector = '.swiper-btn-prod--next',
				prevBtnSelector = '.swiper-btn-prod--prev';
			var pagingTemplete = '<a href="#" class="{{class}}">{{index}}</span>',
				mainSwiper = null;
			...
			if( $target.find('.swiper-slide').length <= 1 ){
				$target.find('.swiper-btn--prev, .swiper-btn--next').hide();
			}
